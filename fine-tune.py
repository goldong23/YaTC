...
from util.datasets import RecursiveImageDataset
...
#20
    ...
    parser.add_argument('--label_level', default=1, type=int,
                        help='1-based directory depth under train/ or test/ to use as the class label')
...
def build_dataset(is_train, args, class_to_idx=None):
    mean = [0.5]
    std = [0.5]
    ...
    print(
        f"Loaded {len(dataset)} {split} images from {root} across {len(dataset.classes)} classes "
        f"(label_level={args.label_level})"
    )
...
def main(args):
    ...
    dataset_val = build_dataset(is_train=False, args=args, class_to_idx=dataset_train.class_to_idx)
